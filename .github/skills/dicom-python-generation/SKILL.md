...
- Refresh generated CodeSystems and ValueSets in `input-src/fsh/`.
...
6. Review the files written under `input-src/fsh/` and `input/resources/` and confirm that only the expected artifacts changed.
...
## Completion Checks
- `input-src/fsh/` contains the regenerated FSH files expected for the requested DICOM scope.
