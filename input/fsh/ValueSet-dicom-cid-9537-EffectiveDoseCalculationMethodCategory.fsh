ValueSet    : CID_9537
Id          : dicom-cid-9537-EffectiveDoseCalculationMethodCategory
Description :
"""
Effective Dose Calculation Method Category

The content in this ValueSet is based on [CID 9537](https://dicom.nema.org/medical/dicom/current/output/chtml/part16/sect_CID_9537.html).
"""
* ^identifier.system = "urn:ietf:rfc:3986"
* ^identifier.value  = "urn:oid:1.2.840.10008.6.1.1247"
* ^version = "20181112"
* ^title = "Effective Dose Calculation Method Category (CID 9537)"
* ^name = "EffectiveDoseCalculationMethodCategory"
* ^experimental = false
* DICOM_DCM_CodeSystem#130126 //"Radiation transport-based methods" 

* DICOM_DCM_CodeSystem#130127 //"Fractionation-based or temporally-based methods" 

