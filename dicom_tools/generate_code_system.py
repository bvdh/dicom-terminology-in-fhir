...
from dicom_spec_parser import get_data_elements, get_uid_values, get_transfer_syntaxes, get_sop_classes, get_vr_values
...
RESOURCE_VALUES = {
    'data_elements': {
        ...
        'specific': [ 
            ...
            '* ^property[=].description = "true when the concept is retired"', 
...
def main(args=None):
    ...
    parser.add_argument("--dicom_resource", help="DICOM Code System or Value Set to extract", type=str, required=True,
                        choices=['data_elements', 'uids', 'transfer_syntaxes', 'sop_classes', 'value_representation'])
    ...
    with open(os.path.join(fsh_path, fsh_filename), 'w') as fsh_file:
        ...
        if resource_values["specific"] :
            for line in resource_values["specific"]:
                fsh_file.write(line+'\n')	
        ...
        for value in value_list:
            if args.dicom_resource == 'data_elements':
                ...
                fsh_file.write(f'* #{value[0]} ^property[2].valueBoolean = {value[6]} ')
